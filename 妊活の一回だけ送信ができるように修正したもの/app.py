...
# ====== 設定 ======
ALLOW_RETAKE = False  # True にすると再問診を許可
...
def is_locked(user_id: str) -> bool:
    """問診完了後は自動応答を止める（=通常チャットモード）"""
    if user_id in completed_users:
        return not ALLOW_RETAKE
    return False

def forward_chat_to_office(user_id: str, text: str):
    """問診完了者の自由チャットを事務局にメール転送（不要なら呼び出しを外す）"""
    try:
        nickname = line_bot_api.get_profile(user_id).display_name
    except Exception:
        nickname = "ご利用者様"

    subject = "【チャット転送】ユーザーからのメッセージ"
    body = (
        "ユーザーからメッセージを受信しました（問診完了者の通常チャット）。\n\n"
        f"ユーザーID: {user_id}\n"
        f"表示名: {nickname}\n\n"
        f"本文:\n{text}\n"
    )
...
def start_registration(user_id, reply_token):
    user_states[user_id] = {}
    completed_users.pop(user_id, None)  # 再開時は過去完了をクリア（ALLOW_RETAKE 時想定）
    try:
...
def handle_follow(event):
    ...
    # すでに問診完了済みなら開始しない（通常チャット案内）
    if is_locked(uid):
        try:
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="すでに問診は完了しています。以降はこのトークで事務局と通常チャットをご利用ください。")
            )
        except Exception as e:
...
def handle_text(event):
    ...
    # 再問診（任意機能）
    if text in ("再問診", "再登録") and ALLOW_RETAKE:
        greeted_users.add(user_id)
        start_registration(user_id, event.reply_token)
        return

    # --- (2) 完了ユーザーは通常チャットモード（転送＆簡易返信） ---
    if is_locked(user_id):
        # 事務局へ転送（不要なら次行を削除）
        forward_chat_to_office(user_id, text)
        # ユーザーへ受領返信（不要なら次行を削除）
        try:
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="メッセージを事務局に転送しました。しばらくお待ちください。")
            )
        except Exception as e:
            print("[Locked-user reply error]", e)
        return

    # --- (3) フォールバック：Follow取りこぼし時の初回発言で開始 ---
    ...
    if step in {"アルコール", "副腎皮質ホルモン剤", "がん", "糖尿病", "その他病気"}:
...
def handle_postback(event):
    ...
    if is_locked(user_id):
        try:
            ...
            print("[Locked-user postback reply error]", e)
        return
...
def finalize_response(event, user_id, state):
    ...
    # 翌朝送信用に完了時刻を保存（→ schedule_daily_followup が配信し、送信後にcompletedから除外）
